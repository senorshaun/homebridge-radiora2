...
module.exports = class OccupancySensor {
    ...
    constructor(log, config, accessory, radiora2, homebridge) {
        ...
        this.accessory
            ...
            .setCharacteristic(Characteristic.Model, this.config.model || "Occupancy Sensor")
    ...
    setupListeners() {
        ...
        this.radiora2.on("groupOccupied", function (integrationId) {
            ...
            if (integrationId == this.config.id) {
                this.log.debug("Group #" + this.config.id + " - '" + this.config.name + "' became OCCUPIED");
                // Occupancy
                this.accessory
        ...
        this.radiora2.on("groupUnoccupied", function (integrationId) {
            // Is this for us?
            if (integrationId == this.config.id) {
                this.log.debug("Group #" + this.config.id + " - '" + this.config.name + "' became UNOCCUPIED");
                // Occupancy
                this.accessory
                    .getService(OccupancySensorService)
                    .getCharacteristic(Characteristic.OccupancyDetected)
        ...
        this.radiora2.on("groupUnknown", function (integrationId) {
            // Is this for us?
            if (integrationId == this.config.id) {
                this.log.debug("Group #" + this.config.id + " - '" + this.config.name + "' became UNKNOWN");
                // Occupancy
                this.accessory
                    .getService(OccupancySensorService)
                    .getCharacteristic(Characteristic.OccupancyDetected)
