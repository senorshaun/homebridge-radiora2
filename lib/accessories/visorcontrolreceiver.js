...
module.exports = class VisorControlReceiver {
    ...
    initializeControls(controlArray, initType) {
        ...
        controlArray.forEach(function(controlConfig){
            ...
            let controlService = this.accessory.getServiceById(Service.Switch, label);
            if (!controlService) {
                let controlService = this.accessory.addService(Service.Switch, label, label);
            }
            this.accessory
                .getServiceById(Service.Switch, label)
                .existsInConfig = true;
        }.bind(this));

        this.accessory.services.forEach(function(accessoryService) {
            var thisControl = this.accessory.getServiceById(Service.Switch, accessoryService.displayName);
    ...
    setupListeners() {
		...
		inputArray.forEach(function(buttonConfig){
			...
			this.accessory
				.getServiceById(Service.Switch, buttonConfig.name)
            ...
            if (integrationId == this.config.id) {
                ...
                inputArray.forEach(function(buttonConfig){
					if(buttonId == buttonConfig.led || buttonId == buttonConfig.id){
						...
						var service = this.accessory.getServiceById(Service.Switch, buttonConfig.name);
						if (service) {
