...
module.exports = class HVACController {
    ...
    setupListeners() {
        ...
        this.radiora2.on("setpoints", function (integrationId, sph, spc) {
            ...
            if (integrationId == this.config.id) {
                ...
                if (sph != "255" && sph != "0"){
                    if (sph > 25) {sph == 25;}
                    this.log.debug("Heating setpoint - " + sph, this.config.name);
                    this.accessory
                        .getService(Service.Thermostat)
                        .getCharacteristic(Characteristic.HeatingThresholdTemperature)
                        .updateValue(sph);
                    if (this.accessory.getService(Service.Thermostat).getCharacteristic(Characteristic.TargetHeatingCoolingState).value == 1) {
                        this.accessory
                            .getService(Service.Thermostat)
                            .getCharacteristic(Characteristic.TargetTemperature)
                            .updateValue(sph);
                ...
                if (spc != "255" && spc != "0"){
                    if (spc < 10) {spc == 10;}
                    this.log.debug("Cooling setpoint - " + spc, this.config.name);
                    this.accessory
                        .getService(Service.Thermostat)
                        .getCharacteristic(Characteristic.CoolingThresholdTemperature)
                        .updateValue(spc);
                    if (this.accessory.getService(Service.Thermostat).getCharacteristic(Characteristic.TargetHeatingCoolingState).value == 2) {
                        this.accessory
                            .getService(Service.Thermostat)
                            .getCharacteristic(Characteristic.TargetTemperature)
                            .updateValue(spc);
        ...
        this.radiora2.on("mode", function (integrationId, mode) {
            ...
            if (integrationId == this.config.id) {
                ...
                this.log.debug("Mode set to " + cleanmode, this.config.name);
                this.accessory
                    .getService(Service.Thermostat)
                    .getCharacteristic(Characteristic.TargetHeatingCoolingState)
        ...
        this.radiora2.on("state", function (integrationId, state) {
            ...
            if (integrationId == this.config.id) {
                if(["0","5","8"].indexOf(state) > -1) {
                    this.log.debug("Currently off", this.config.name);
                    this.accessory
                        .getService(Service.Thermostat)
                        .getCharacteristic(Characteristic.CurrentHeatingCoolingState)
                        .updateValue(0);
                } else if(["1","2","3","4","5","9"].indexOf(state) > -1) {
                    this.log.debug("Currently heating", this.config.name);
                    this.accessory
                        .getService(Service.Thermostat)
                        .getCharacteristic(Characteristic.CurrentHeatingCoolingState)
                        .updateValue(1);
                } else if (["6","7","10"].indexOf(state) > -1) {
                    this.log.debug("Currently cooling", this.config.name);
                    this.accessory
                        .getService(Service.Thermostat)
                        .getCharacteristic(Characteristic.CurrentHeatingCoolingState)
                        .updateValue(2);
        ...
        // Sensors
        this.radiora2.on("sensors", function (integrationId, sensorsState) {
            // Is this for us?
            if (integrationId == this.config.id && sensorsState == 2) {
                (this.config.sensors || []).forEach(function (sensor) {
                    if (sensor.id) {
                        this.radiora2.queryTemperatureSensor(sensor.id);
                    }
                });
            }
        }.bind(this));
    ...
    setTargetTemperature(sp, callback) {
        ...
        } else if (this.accessory.getService(Service.Thermostat).getCharacteristic(Characteristic.TargetHeatingCoolingState).value == 2) {
