...
module.exports = class TemperatureSensor {
    ...
    constructor(log, config, accessory, radiora2, homebridge) {
        ...
        let BatteryService = this.accessory.getServiceById(Service.Battery, 'Battery');
        if (!BatteryService) {
            BatteryService = this.accessory.addService(Service.Battery, 'Battery', 'Battery');
        }
        let batterySwitch = this.accessory.getServiceById(Service.Switch, 'BatterySwitch');
    ...
    setupListeners() {
        ...
        this.radiora2.on("temp", function (integrationId, currentTemp) {
            ...
            if (integrationId == this.config.id || integrationId == this.config.parentId) {
                ...
                if(this.accessory.getService(Service.TemperatureSensor).getCharacteristic(Characteristic.StatusActive).value == 1) {
    ...
    setState(active, lowBattery, fault) {
        ...
        if (this.config.batterySwitch) {
            this.accessory
                .getServiceById(Service.Switch, "BatterySwitch")
