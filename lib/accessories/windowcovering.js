...
module.exports = class WindowCovering {
    ...
    setHoldPosition(level, callback) {
        this.setPostionState(this.accessory.getService(WindowCoveringService).getCharacteristic(Characteristic.CurrentPosition));
        setTimeout(function() {
            if (this.config.blind) {
    ...
    setPostionState(level) {
        if (level > this.accessory.getService(WindowCoveringService).getCharacteristic(Characteristic.CurrentPosition)) {
            this.accessory
                .getService(WindowCoveringService)
                .setCharacteristic(Characteristic.PositionState, Characteristic.PositionState.INCREASING);
        } else if (level < this.accessory.getService(WindowCoveringService).getCharacteristic(Characteristic.CurrentPosition)) {
            this.accessory
                .getService(WindowCoveringService)
                .setCharacteristic(Characteristic.PositionState, Characteristic.PositionState.DECREASING);
        } else if (level == this.accessory.getService(WindowCoveringService).getCharacteristic(Characteristic.CurrentPosition)) {
            this.accessory
                .getService(WindowCoveringService)
