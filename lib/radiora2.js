...
var RadioRa2 = function (host, username, password) {
    ...
    function _processHVACResponse(dataComponents) {
        ...
        if ((dataComponents.length >= 3)) {
            ...
            // current single setpoint
            else if (action == 19) {
                me.emit("singlesetpoint", deviceId, parameter);
            }
            // mode
            else if (action == 3) {
    ...
    this.setHVACSingleSetpoint = function (id, sp, drift){
        me.sendCommand("#HVAC," + id + ",19," + sp + "," + drift + "," + drift);
    };
