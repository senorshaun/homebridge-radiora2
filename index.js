...
function addDefaultValues(deviceConfig, deviceType) {
  deviceConfig.name = (
    deviceConfig.name || deviceType + " " + deviceConfig.id
  ).toString();
  deviceConfig.model = (
    deviceConfig.model || "RadioRa2-" + deviceType
  ).toString();
  deviceConfig.serial = (
    deviceConfig.serial || deviceType + deviceConfig.id
  ).toString();
  return deviceConfig;
}
  ...
  setupListeners() {
    ...
    this.radiora2.on(
      ...
      function () {
            ...
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                if (!deviceAccessory) {
                  ...
                  let deviceService = accessory.addService(
                    Service.Fan,
                    deviceConfig.name,
                  );
                ...
                this.accessories[uuid] = new FanAccessory(
                  this.log,
                  deviceConfig,
                  deviceAccessory instanceof FanAccessory
                    ? deviceAccessory.accessory
                    : deviceAccessory,
                  this.radiora2,
                  Homebridge,
                ...
                this.log.debug(
                  "Loaded " + deviceType + " '" + deviceConfig.name + "'",
                );
              } else {
                this.log.warn(
                  "Invalid " + deviceType + " in config. Not loading it.",
                );
              }
            }
          }.bind(this),
          ...
          function (deviceConfig) {
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                this.accessories[uuid] = new LightbulbAccessory(
                  this.log,
                  deviceConfig,
                  deviceAccessory instanceof LightbulbAccessory
                    ? deviceAccessory.accessory
                    : deviceAccessory,
                  this.radiora2,
                  Homebridge,
            ...
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                if (!deviceAccessory) {
                  ...
                  deviceService.addOptionalCharacteristic(
                ...
                this.accessories[uuid] = new OccupancySensorAccessory(
                  this.log,
                  deviceConfig,
                  deviceAccessory instanceof OccupancySensorAccessory
                    ? deviceAccessory.accessory
                    : deviceAccessory,
                  this.radiora2,
                  Homebridge,
            ...
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                if (deviceConfig.stateless) {
                  this.accessories[uuid] = new KeypadButtonStatelessAccessory(
                    this.log,
                    deviceConfig,
                    deviceAccessory instanceof KeypadButtonStatelessAccessory
                      ? deviceAccessory.accessory
                      : deviceAccessory,
                    this.radiora2,
                    Homebridge,
                  ...
                  this.accessories[uuid] = new KeypadButtonAccessory(
                    this.log,
                    deviceConfig,
                    deviceAccessory instanceof KeypadButtonAccessory
                      ? deviceAccessory.accessory
                      : deviceAccessory,
                    this.radiora2,
                    Homebridge,
          ...
          function (deviceConfig) {
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                this.accessories[uuid] = new VisorControlReceiverAccessory(
                  this.log,
                  deviceConfig,
                  deviceAccessory instanceof VisorControlReceiverAccessory
                    ? deviceAccessory.accessory
                    : deviceAccessory,
                  this.radiora2,
                  Homebridge,
          ...
          function (deviceConfig) {
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                this.accessories[uuid] = new ThermostatAccessory(
                  this.log,
                  deviceConfig,
                  deviceAccessory instanceof ThermostatAccessory
                    ? deviceAccessory.accessory
                    : deviceAccessory,
                  this.radiora2,
                  Homebridge,
                ...
                subdeviceArray.forEach(function (subdeviceConfig) {
                  if (!subdeviceConfig.disabled) {
                    if (subdeviceConfig.id) {
                      ...
                      subdeviceConfig = addDefaultValues(
                        subdeviceConfig,
                        subdeviceType,
                      );
                      var subuuid = UUIDGen.generate(
                        subdeviceType + ":" + subdeviceConfig.id,
                      );
                      let subdeviceAccessory = that.accessories[subuuid];
                      if (!subdeviceAccessory) {
                        let subaccessory = new PlatformAccessory(
                          subdeviceConfig.name,
                          subuuid,
                        );
                        let subdeviceService = subaccessory.addService(
                      ...
                      that.accessories[subuuid] =
                        new TemperatureSensorAccessory(
                          that.log,
                          subdeviceConfig,
                          subdeviceAccessory instanceof
                            TemperatureSensorAccessory
                            ? subdeviceAccessory.accessory
                            : subdeviceAccessory,
                          that.radiora2,
                          Homebridge,
                        );
          ...
          function (deviceConfig) {
            if (!deviceConfig.disabled) {
              if (deviceConfig.id) {
                ...
                this.accessories[uuid] = new WindowCoveringAccessory(
                  this.log,
                  deviceConfig,
                  deviceAccessory instanceof WindowCoveringAccessory
                    ? deviceAccessory.accessory
                    : deviceAccessory,
                  this.radiora2,
                  Homebridge,
        ...
        Object.keys(this.accessories).forEach(
          function (accessoryUuid) {
            var thisAccessory = this.accessories[accessoryUuid];
            if (thisAccessory.existsInConfig !== true) {
              this.api.unregisterPlatformAccessories(undefined, undefined, [
                thisAccessory,
              ]);
